...
``` bash
npm install express body-parser xml2js http-z --no-save
```
Run sample
``` bash
npm run sample
```
Open browser to http://localhost:3000/sample.htm

Sample application will generate WSMan Messages that can be sent to a target AMT device.  The sample app currently only supports talking to AMT using the unencrypted port (16992).