...
* @author Ylian Saint-Hilaire / Joko Sastriawan / Craig Spencer
* @version v0.2.0b
*/

// Construct a MeshServer object
export function CreateWsmanComm (host, port, user, pass, tls, tlsoptions, setupCommunication) {
    ...
    obj.pass = pass;
    obj.xtls = tls;
    obj.setupCommunication = setupCommunication;
    obj.xtlsoptions = tlsoptions;
    obj.xtlsFingerprint = 0;
    ...
    obj.xxConnectHttpSocket = function () {
        ...
        obj.socketState = 1;
        obj.kerberosDone = 0;
        obj.socket = obj.setupCommunication(obj.host, obj.port);
    ...
    obj.xxOnSocketConnected = function () {
        ...
        if (obj.xtls == 1) {
            ...
            if (obj.xtlsoptions.ca) {
                var caStore = pki.createCaStore([obj.xtlsoptions.ca]);
                var forgeCert = pki.certificateFromAsn1(asn1.fromDer(atob(obj.xtlsCertificate.raw.toString('base64'))));
    ...
    obj.xxOnSocketError = function(err){
        console.log(err);
    }
    ...
    obj.xxProcessHttpResponse = function (header, data) {
        ...
        if (s == 401 && ++(obj.authcounter) < 3) {
            ...
            if (obj.setupCommunication == null || obj.setupCommunication == undefined) {
                obj.socket.end();
            }
        } else {
