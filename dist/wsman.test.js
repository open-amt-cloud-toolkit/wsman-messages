...
const wsmanMessageCreator = new WSMan_1.WSManMessageCreator();
describe('WSManMessageCreator Tests', () => {
    ...
    describe('createXml Tests', () => {
        ...
        it('should throw error if header is null/undefined in createXml', () => {
            const header = null;
            const body = wsmanMessageCreator.createBody('Enumerate');
            expect(() => { wsmanMessageCreator.createXml(header, body); }).toThrow(WSMan_1.WSManErrors.HEADER);
        });
        it('should throw error if body is null/undefined in createXml', () => {
            const header = wsmanMessageCreator.createHeader('http://schemas.xmlsoap.org/ws/2004/09/enumeration/Pull', 'http://schemas.dmtf.org/wbem/wscim/1/cim-schema/2/CIM_ServiceAvailableToElement', messageId);
            const body = null;
            expect(() => { wsmanMessageCreator.createXml(header, body); }).toThrow(WSMan_1.WSManErrors.BODY);
        });
    });
    describe('createHeader Tests', () => {
        ...
        it('should throw error if missing action in createHeader', () => {
            expect(() => { wsmanMessageCreator.createHeader(null, 'http://schemas.dmtf.org/wbem/wscim/1/cim-schema/2/CIM_ServiceAvailableToElement', messageId); }).toThrow(WSMan_1.WSManErrors.ACTION);
        });
        it('should throw error if missing resourceUri in createHeader', () => {
            expect(() => { wsmanMessageCreator.createHeader('http://schemas.xmlsoap.org/ws/2004/09/enumeration/Enumerate', null, messageId); }).toThrow(WSMan_1.WSManErrors.RESOURCE_URI);
        });
        it('should throw error if missing messageId in createHeader', () => {
            expect(() => { wsmanMessageCreator.createHeader('http://schemas.xmlsoap.org/ws/2004/09/enumeration/Enumerate', 'http://schemas.dmtf.org/wbem/wscim/1/cim-schema/2/CIM_ServiceAvailableToElement', null); }).toThrow(WSMan_1.WSManErrors.MESSAGE_ID);
        });
    ...
    describe('createBody Tests', () => {
        ...
        it('should throw error if Pull is missing enumerationContext in createBody', () => {
            expect(() => { wsmanMessageCreator.createBody('Pull'); }).toThrow(WSMan_1.WSManErrors.ENUMERATION_CONTEXT);
        });
        it('should throw error if RequestStateChange is missing input in createBody', () => {
            expect(() => { wsmanMessageCreator.createBody('RequestStateChange', null, null, 8); }).toThrow(WSMan_1.WSManErrors.INPUT);
        });
        it('should throw error if RequestStateChange is missing requestedState in createBody', () => {
            expect(() => { wsmanMessageCreator.createBody('RequestStateChange', null, 'http://intel.com/wbem/wscim/1/amt-schema/1/AMT_UserInitiatedConnectionService', null); }).toThrow(WSMan_1.WSManErrors.REQUESTED_STATE);
        });
        it('should throw error if method is not handled in createBody', () => {
            expect(() => { wsmanMessageCreator.createBody('test'); }).toThrow(WSMan_1.WSManErrors.UNSUPPORTED_METHOD);
        });
    });
